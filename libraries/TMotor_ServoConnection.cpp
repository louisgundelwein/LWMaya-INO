...
void TMotor_ServoConnection::parse_CAN_data(long unsigned int id, unsigned char rx_message[8])
{
  ...
  { // change values of existing entry
...
void TMotor_ServoConnection::wait_until_reached(long unsigned int id, float pos)
{
  while (abs(get_pos(id) - pos) > 0.01)
  {            // Toleranz von 0,01 Einheiten
    delay(10); // Kurze Verzögerung zwischen den Überprüfungen
  }
}
